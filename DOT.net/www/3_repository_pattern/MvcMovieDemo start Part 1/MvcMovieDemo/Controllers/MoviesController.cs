...
namespace MvcMovieDemo.Controllers
{
    public class MoviesController : Controller
    {
        ...
        public MoviesController(MovieContext context)
        {
            ...
            _movieRepository = new GenericRepository<Movie>(context);
        ...
        public IActionResult List(int ratingID = 0)
        {
            ...
            if (ratingID != 0)
            {
                listMoviesVM.Movies = _movieRepository.Get(filter: m => m.RatingID == ratingID, orderBy: m => m.OrderBy(x => x.Title)).ToList();
            }
            else
            {
                listMoviesVM.Movies = _movieRepository.Get(orderBy: m => m.OrderBy(x => x.Title)).ToList();
            }

            // This wil get all items, and then filter.
            //listMoviesVM.Ratings = new SelectList(_ratingRepository.GetAll().OrderBy(r => r.Name),
            //            "RatingID", "Name");
            //listMoviesVM.ratingID = ratingID;


            listMoviesVM.Ratings = new SelectList(_ratingRepository.Get(orderBy: m => m.OrderBy(x => x.Name)),
                        "RatingID", "Name");
            listMoviesVM.ratingID = ratingID;
        ...
        public IActionResult Details(int id)
        {
            ...
            var movie = _movieRepository.Get(filter: x => x.MovieID == id, includes: x => x.Rating).FirstOrDefault();
        ...
        public IActionResult Create([Bind("MovieId,Title, Genre,ReleaseDate, RatingID")] Movie movie)
        {
            ...
            ViewData["Ratings"] = new SelectList(_ratingRepository.Get(orderBy: m => m.OrderBy(x => x.Name)),
                           "RatingID", "Name");
